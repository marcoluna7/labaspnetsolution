...
//  WBToggleCheck:       Checa/Descheca checkbox hijos, abre/colapsa filtro y agrega botón de ver más
//  Dependencias:   jQuery.js (1.8.3) http://jquery.com,

// ---------------------------------------------------------------------------------------------------------
	var WBFilterToggle = function(selector, params){
		// El objeto
		var _this = this;
		// Configuración base
	    _this.defaults = $.extend({
	    	// Custom Checkbox
	    	doCheckbox: false,
	    	customCheckCheckbox: 'input[type="checkbox"]',
			customCheckSelectClass: 'checkbox-checked',
			customCheckUnSelectClass: 'checkbox-unchecked',
			customCheckWrapper: 'checkbox-wrapper',
			customCheckContainer: 'checkbox-span',
			customCheckOnClickCall: false,
			customCheckOnFinishWrapping: false,
			// Acordeon
			doAcordeon: false,
			acordeonInitClose: true, // true: Cerrado, false: False
			acordeonTrigger: '.openAcordeon',
			acordeonActiveClass: 'iconFont-up',
			acordeonUnactiveClass: 'iconFont-down',
			acordeonContainer: '.acordeon-sub',
			// Toggle check
			doToggle: false,
			toggleCheckTitle: 'h4',
			toggleCheckSubContainer: '.acordeon-subContainer',
			toggleCheckSubContainerTitle: '.acordeon-subContainer-title',
			toggleCheckSubContainerContent: '.acordeon-subsub',
			// Add More Button
			doAddMoreButton: false,
			addMoreItems: '.checkbox-wrapper',
			addMoreContador: 5,
			addMoreContainer: 'add-more',
			addMoreIcon: 'add-more-icon',
			addMoreMoreIcon: 'iconFont-down',
			addMoreLessIcon: 'iconFont-up',
			addMoreMoreText: 'más ',
			addMoreLessText: 'menos ',
			addMoreClass: 'add-more-txt',
			addMoreHide: 'add-more-hide',
			addMoreHasChild: false
	    }, params);
	    // MÉTODOS
	   // 1. Activar el filtro
	    _this.activateFilterToggle = function(){
			// Verificar que no vengan atributos adicionales data en el selector
			_this.checkData(selector);
			// Si esta en true el checkbox, ejecutar el customcheckbox
			if(_this.defaults.doCheckbox) {
				_this.customCheckbox.doCustomCheckbox();
			}
			// Si esta en true el acordeón, ejecutar el acordeón
			if(_this.defaults.doAcordeon){
				_this.acordeon.doAccordeon();
			}
			// Si esta en true el toggle check, ejecutar el toggle check
			if(_this.defaults.doToggle && _this.defaults.acordeonInitClose){
				setTimeout(function(){
					_this.toggleCheck.doToggleCheck();
				}, 500);
			}
			// Si esta en true el add more button, ejecutar el acordeón
			if(_this.defaults.doAddMoreButton){
				_this.addMoreButton.doAddMoreButton();
			}
	    };
	    // 2. Verificar si existen data en el objeto para modificar los defaults
	    _this.checkData = function(obj){
	        $.extend(_this.defaults, $(obj).data());
	    };
	    // 3. Custom checkbox
	    _this.customCheckbox = {
	    	// 3.0 Ejecutar custom Checkbox
	    	doCustomCheckbox: function(){
	    		$(selector).find(_this.defaults.customCheckCheckbox).each(function(){
	    			_this.customCheckbox.wrappingInput(this);
	    		});
	    	},
			// 3.1 Envolverl el input
	    	wrappingInput: function(obj){
	    		// ¿No existe el wrapper?
	    		if(!$(obj).parent(_this.defaults.customCheckWrapper).length){
	    			// ¿El siguiente elemento es un tag tipo label?
	    			if($(obj).next().is('label')){
	    				// El label y el input, envuélvelos en un contenedor div
						$(obj).next().andSelf().wrapAll('<div class="'+ _this.defaults.customCheckWrapper +'"/>');
						// Cuando le hagan click al label, quitar acción por default
						$(obj).next().click(function(e){
							e.preventDefault();
						});
						// Mueve el input dentro del label
						$(obj).appendTo($(obj).next());
					// Si el siguiente elemento no es label
					} else {
						// Envuelve el input en un contenedor div
						$(obj).wrap('<div class="' + _this.defaults.customCheckWrapper + '"/>');
					}
					// Ya que esta envuelo, al wrapper insertale el elemento estilizado para dar click
					$(obj).parents('.'+_this.defaults.customCheckWrapper).prepend('<span class="'+ _this.defaults.customCheckContainer +'"/>');
					// Verifica si esta checado o no para poner las clases
					_this.customCheckbox.checkingChecked(obj);
					// Cuando le den click al wrapper, ejecutar el método clickingCheckbox
					$(obj).parents('.'+_this.defaults.customCheckWrapper).click(function(){
						_this.customCheckbox.clickingCheckbox(this);
					});
	    		}
			},
			// 3.2 Checar checkbox si esta seleccionado o no
	    	checkingChecked: function(obj){
	    		var $wrapper = $(obj).parents('.'+_this.defaults.customCheckWrapper),
	    			$container = $wrapper.find('.'+_this.defaults.customCheckContainer);
	    			// ¿Esta checkado? Agregar clases de checado y eliminar clase de no checkado
				if($(obj).prop('checked')){
					$wrapper.removeClass(_this.defaults.customCheckUnSelectClass).
					addClass(_this.defaults.customCheckSelectClass);
					$container.removeClass(_this.defaults.customCheckUnSelectClass).
					addClass(_this.defaults.customCheckSelectClass);
					$(obj).removeClass(_this.defaults.customCheckUnSelectClass).
					addClass(_this.defaults.customCheckSelectClass);
					// ¿No esta checkado? Agregar clases de no checado y eliminar clase de checkado
				} else {
					$wrapper.removeClass(_this.defaults.customCheckSelectClass).
					addClass(_this.defaults.customCheckUnSelectClass);
					$container.removeClass(_this.defaults.customCheckSelectClass).
					addClass(_this.defaults.customCheckUnSelectClass);
					$(obj).removeClass(_this.defaults.customCheckSelectClass).
					addClass(_this.defaults.customCheckUnSelectClass);
				}
			},
			// 3.3 Método para el click en el checkbox
			clickingCheckbox: function(obj){
				var $input = $(obj).find(_this.defaults.customCheckCheckbox);
				if($input.prop('checked')){
					$input.prop('checked', false);
				} else {
					$input.prop('checked', true);
				}
				_this.customCheckbox.checkingChecked($input);
				if(_this.defaults.customCheckOnClickCall){
					_this.defaults.customCheckOnClickCall($input);
				}
				_this.toggleCheck.initToggle($input);
			}
	    };
	    // 4. Acordeon
	    _this.acordeon = {
	    	// 4.1 Ejecutar acordeon
	    	doAccordeon: function(){
	    		$(selector).find(_this.defaults.acordeonContainer).each(function(){
	    			// Si el contenedor del acordeón tiene algún hijo, iniciar el acordeón
	    			if($(this).children().length > 0){
	    				_this.acordeon.initAcordeon(this, 1);
	    			// Si no tiene hijos, eliminar del DOM objetos del acordeón
	    			} else {
	    				_this.acordeon.removeAcordeon(this);
	    			}
	    		});
	    	},
	    	// 4.2 Remover el acordeón
	    	removeAcordeon: function(obj){
	    		// Quitar el DOM del trigger y el contenedor
				$(obj).find(_this.defaults.acordeonTrigger).remove();
				$(obj).find(_this.defaults.acordeonContainer).remove();
	    	},
	    	// 4.3 Iniciar el acordeón
	    	initAcordeon: function(obj){
	    		// Si al inicio debe estar cerrado o abierto (default = cerrado)
				if(_this.defaults.acordeonInitClose){
					// El acordeón estará cerrado
					_this.acordeon.initStatus(obj,1);
				// Si debe estar abierto
				} else {
					_this.acordeon.initStatus(obj);
				}
				// Inicia el evento click en el trigger
				_this.acordeon.clickTrigger(obj);
	    	},
	    	// 4.4 Método para abrir/cerrar el acordeón 
			initStatus: function(obj, status){
				// Status = 1 : cerrado, Status = '' : abierto
				if (status) {
					$(obj).parents(selector).find(_this.defaults.acordeonTrigger).
					addClass(_this.defaults.acordeonUnactiveClass).
					removeClass(_this.defaults.acordeonActiveClass);
					$(obj).parents(selector).find(_this.defaults.acordeonContainer).stop().slideUp();
				} else {
					$(obj).parents(selector).find(_this.defaults.acordeonTrigger).
					addClass(_this.defaults.acordeonActiveClass).
					removeClass(_this.defaults.acordeonUnactiveClass);
					$(obj).parents(selector).find(_this.defaults.acordeonContainer).stop().slideDown();
				}
			},
			// 4.5 Método para abrir/cerrar en el click en el trigger
			clickTrigger: function(obj){
				$(obj).parents(selector).find(_this.defaults.acordeonTrigger).click(function(){
					// Si tiene la clase activo
					if($(this).hasClass(_this.defaults.acordeonActiveClass)){
						// Cierra el acordeón
						_this.acordeon.initStatus(obj, 1);
					// Si no tiene la clase activo
					} else {
						// Abre el acordeón
						_this.acordeon.initStatus(obj);
					}
	    ...
	    // 5. Toggle Check
	    _this.toggleCheck = {
	    	// 5.1 Ejecutar Toggle Check
	    	doToggleCheck: function(){
	    		// Verificamos cada input checkbox
	    		$(selector).find(_this.defaults.customCheckCheckbox).each(function(){
	    			// Si el algun input esta checado, muestra al contenedor del acordeón
	    			if($(this).prop('checked')){
	    				$(this).parents(selector).find(_this.defaults.acordeonContainer).slideDown();
	    				$(this).parents(selector).find(_this.defaults.acordeonTrigger).
						addClass(_this.defaults.acordeonActiveClass).
						removeClass(_this.defaults.acordeonUnactiveClass);
						$(this).parents(selector).find(_this.defaults.toggleCheckSubContainerContent).slideDown();
					}
	    		});
	    	},
	    	// 5.2 Contamos todos los inputs del selector checados
	    	countCheckedInputs: function(obj){
	    		var $inputs = $(obj),
	    		// Contador de inputs
	    			count =  0;
	    		// Iterar por todos los inputs del selector y contar cuántos estan checados
				$inputs.each(function(){
					if($(this).prop('checked')){
						count = count + 1;
					}
				});
				// Regresa los inputs checados
				return count;
	    	},
	    	// 5.3 Iniciar toggle check
	    	initToggle: function(obj){
	    		// Buscar todos los inputs del contenedor
	    		var $inputAllSiblings = $(obj).parents(selector).find(_this.defaults.customCheckCheckbox),
					// Buscar a los inputs hermanos dentro del sub acordeón
					$inputSiblings = $(obj).parents(selector).find(_this.defaults.toggleCheckSubContainerContent).find(_this.defaults.customCheckCheckbox),
					// Contador de inputs del sub acordeon checados
					countAllChecked = _this.toggleCheck.countCheckedInputs($inputAllSiblings),
					// Contador de inputs del selector checados
					countChecked = _this.toggleCheck.countCheckedInputs($inputSiblings),
					// Contador de inputs del selector
					countAllInputs =  $inputAllSiblings.length,
					// Contador de inputs del sub acordeon
					// countInputs =  $inputSiblings.length,
					// Título
					$titleInput = $(obj).parents(selector).find(_this.defaults.toggleCheckTitle).find(_this.defaults.customCheckCheckbox),
					// Subtitulo
					$subTitleInput = $(obj).parents(selector).find(_this.defaults.toggleCheckSubContainerTitle).find(_this.defaults.customCheckCheckbox);
	    		// Si está dentro de un título (h4)
	    		if($(obj).parents(_this.defaults.toggleCheckTitle).length){
	    			// Si esta checado el input
					if($(obj).prop('checked')){
						// Checa todos los inputs dentro del selector padre
						$inputAllSiblings.prop('checked', true);
						// Abre el acordeón
						_this.acordeon.initStatus(obj);
						// Abre cualquier subitem que tenga el acordeón
						$(obj).parents(selector).find(_this.defaults.toggleCheckSubContainerContent).slideDown();
						// Cambiar las clases dentro del wrapper del input
						_this.customCheckbox.checkingChecked($inputAllSiblings);
					// Si el input no esta checado
					} else {
						// Descheca todos los inputs del selector padre
						$inputAllSiblings.prop('checked', false);
						// Colapsa el acordeón
						_this.acordeon.initStatus(obj, 1);
						// Colapsa cualquier subitem que tenga el acordeón
						$(obj).parents(selector).find(_this.defaults.toggleCheckSubContainerContent).slideUp();
						// Cambiar las clases dentro del wrapper del input
						_this.customCheckbox.checkingChecked($inputAllSiblings);
					}
				// Si tienes hijos dentro de un container hermano
	    		} else if($(obj).parents(_this.defaults.toggleCheckSubContainerTitle).length){
					// El input esta checado?
					if($(obj).prop('checked')){
						// Checa todos los inputs desde el div padre
						$inputSiblings.prop('checked', true);
						// Abre el acordeón hijo
						$(obj).parents(selector).find(_this.defaults.toggleCheckSubContainerContent).slideDown();
						// Volvemos a contar los input checados
						countAllChecked = _this.toggleCheck.countCheckedInputs($inputAllSiblings);
						// Si todos los hermanos estan checados, incluyendo el actual input
						if(countAllInputs === (countAllChecked + 1)){
						// 	// Checa el título
							$titleInput.prop('checked', true);
						} else {
							$titleInput.prop('checked', false);
						}
					// El input no esta checado?
					} else {
						// Descheca todos los inputs desde el div padre
						$inputSiblings.prop('checked', false);
						// Descheca el título
						$titleInput.prop('checked', false);
						// Colapsa el acordeón hijo
						$(obj).parents(selector).find(_this.defaults.toggleCheckSubContainerContent).slideUp();
					}
				// Estas dentro de un sub acordeón?
				} else if($(obj).parents(_this.defaults.toggleCheckSubContainerContent).length){
					// El input esta checado?
					if($(obj).prop('checked')){
						// Si todos los inputs del selector estan checados
						if(countAllInputs === (countAllChecked + 2)){
							// Checar el título y el subtítulo
							$titleInput.prop('checked', true);
							$subTitleInput.prop('checked', true);
						} else {
							// Deschecar el título y el subtítulo
							$titleInput.prop('checked', false);
							$subTitleInput.prop('checked', false);
						}
					} else {
						// Deschecar el título y el subtítulo
						$titleInput.prop('checked', false);
						$subTitleInput.prop('checked', false);
						// El contador de hermanos del sub acordeón es igual a 0
						if(countChecked === 0){
							// Colpsa en sub acordeon
							$(obj).parents(_this.defaults.toggleCheckSubContainerContent).slideUp();
						}
						// El contador de inputs del selector es igual a 0
						if(countAllChecked === 0){
							// Colapsa el acordeón
							_this.acordeon.initStatus(obj, 1);
						}
					}
				// Ni estas en título y no estas dentro de un sub acordeon
				} else {
					// Si el input esta checado
				...
				// Actualiza las clases dentro del wrapper del input del título y del subtítulo
				_this.customCheckbox.checkingChecked($titleInput);
				_this.customCheckbox.checkingChecked($subTitleInput);
				_this.customCheckbox.checkingChecked(obj);
	    	}
	    };
	    // 6. Add More Button
	    _this.addMoreButton = {
	    	// 6.1 Ejecutar Add More Button
	    	doAddMoreButton: function(){
	    		// Buscamos el contenedor del acordeón
	    		$(selector).find(_this.defaults.acordeonContainer).each(function(){
	    			// Si el contenedor del acordeón tiene algún hijo, iniciar el addMore
	    			if($(this).children().length > 0){
	    				_this.addMoreButton.initAddMore(this);
	    			}
	    		});
	    	},
	    	// 6.2 Iniciar add more button
	    	initAddMore: function(obj){
	    		// Buscamos el acordeón y contamos cuántos hijos de addMoreItems
	    		var $container =  $(obj).parents(selector).find(_this.defaults.acordeonContainer),
	    			$lenghtItems = $container.children(_this.defaults.addMoreItems).length;
	    		// Si los hijos son más que el contador
	    		if($lenghtItems > _this.defaults.addMoreContador) {
	    			// Si no encuentras el objeto addMoreContainer
					if(!$container.find('.'+ _this.defaults.addMoreContainer).length) {
						// Agrega un div con los objetos para hacer el ver más
						$container.append('<div class="' + _this.defaults.addMoreContainer + '"><span class="'+ _this.defaults.addMoreClass + '">' + _this.defaults.addMoreMoreText + '</span><span class="' + _this.defaults.addMoreIcon + ' ' +_this.defaults.addMoreMoreIcon + '"></span></div>');
					}
					// Agregamos las clases ocultas a los hijos que exceden el número del contador
					_this.addMoreButton.addMoreClass(obj);
					// Atachamos el evento click al botón ver más
					_this.addMoreButton.clickMore(obj);
				}
			},
			// 6.3 Agregamos/quitamos clases ocultas a los hijos excedentes del contador
			addMoreClass: function(obj){
				// Buscamos los hijos para iterar cada uno
				$(obj).parents(selector).find(_this.defaults.acordeonContainer).children(_this.defaults.addMoreItems).each(function(i){
					// Si el índice del hijo es mayor al contador
					if(i >= _this.defaults.addMoreContador){
						// Switchear entre Ocultar/Mostrar
						$(this).slideToggle(100, function(){
							// Cuando termine de Ocultar/Mostrar, switchea la clasee addMoreHide
							$(this).toggleClass(_this.defaults.addMoreHide);
						});
					}
				});
			},
			// 6.4 Evento de click en el botón ver más
			clickMore: function(obj){
				// Buscamos el botón
				$(obj).parents(selector).find('.' + _this.defaults.addMoreContainer).click(function(e){
					// Eliminar el evento default
					e.preventDefault();
					// Switcheamos los textos y las clases en el botón ver más
					_this.addMoreButton.toggleText(obj);
					// Agregamos/quitamos clases ocultas a los hijos excedentes del contador
					_this.addMoreButton.addMoreClass(obj);
				});
			},
			// Variable que logrará el switch para cambiar los textos
			toggleTxt: true,
			// 6.5 Switchear texto y clases en el botón
			toggleText: function(obj){
				// Si la variable toggleTxt es verdadera, el botón tiene texto de ver más y la clase ver más
				if (_this.addMoreButton.toggleTxt){
					$(obj).parents(selector).find('.' + _this.defaults.addMoreIcon).
							removeClass(_this.defaults.addMoreMoreIcon).
							addClass(_this.defaults.addMoreLessIcon);
					$(obj).parents(selector).find('.' + _this.defaults.addMoreClass).text(_this.defaults.addMoreLessText);
					// Modificar la variable a falso
					_this.addMoreButton.toggleTxt = false;
				// La variable toggleTxt es falsa, el botón tiene texto de ver menos y la clase ver menos
				} else {
					$(obj).parents(selector).find('.' + _this.defaults.addMoreIcon).
							removeClass(_this.defaults.addMoreLessIcon).
							addClass(_this.defaults.addMoreMoreIcon);
					$(obj).parents(selector).find('.' + _this.defaults.addMoreClass).text(_this.defaults.addMoreMoreText);
					// Modificar la variable a verdadero
					_this.addMoreButton.toggleTxt = true;
				}
			}
	    };
	    // Activar el filtro
	    _this.activateFilterToggle();
